				...
				<li class="user-job-package">
					...
					<?php
						if ( $package->get_limit() ) {
							printf( _n( '%s job posted out of %d', '%s jobs posted out of %s', $package->get_count(), 'babysitter' ) . ', ', $package->get_count(), $package->get_limit() );
						} else {
							printf( _n( '%s job posted', '%s jobs posted', $package->get_count(), 'babysitter' ) . ', ', $package->get_count() );
						}
		...
		<?php if ( $packages ) : ?>
			...
			<?php foreach ( $packages as $key => $package ) :
				...
				<li class="job-package">
					...
					<?php
						printf( _n( '%s for %d job', '%s for %s jobs', $product->get_limit(), 'babysitter' ) . ' ', $product->get_price_html(), $product->get_limit() ? $product->get_limit() : __( 'unlimited', 'babysitter' ) );
