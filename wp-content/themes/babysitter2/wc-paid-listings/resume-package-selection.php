<?php if ( $packages || $user_packages ) :
	...
	<ul class="resume_packages">
		<?php if ( $user_packages ) : ?>
			<li class="package-section"><?php _e( 'Your Packages:', 'babysitter' ); ?></li>
				...
				<li class="user-resume-package">
					...
					<?php
						if ( $package->get_limit() ) {
							printf( _n( '%s resume posted out of %d', '%s resumes posted out of %s', $package->get_count(), 'babysitter' ), $package->get_count(), $package->get_limit() );
						} else {
							printf( _n( '%s resume posted', '%s resumes posted', $package->get_count(), 'babysitter' ), $package->get_count() );
						}

						if ( $package->get_duration() ) {
							printf( ' ' . _n( 'listed for %s day', 'listed for %s days', $package->get_duration(), 'babysitter' ), $package->get_duration() );
						}
		...
		<?php endif; ?>
		<?php if ( $packages ) : ?>
			<li class="package-section"><?php _e( 'Purchase Package:', 'babysitter' ); ?></li>
			<?php foreach ( $packages as $key => $package ) :
				...
				<li class="resume-package">
					...
					<?php
						printf( _n( '%s to post %d resume', '%s to post %s resumes', $product->get_limit(), 'babysitter' ) . ' ', $product->get_price_html(), $product->get_limit() ? $product->get_limit() : __( 'unlimited', 'babysitter' ) );

						if ( $product->get_duration() ) {
							printf( ' ' . _n( 'listed for %s day', 'listed for %s days', $product->get_duration(), 'babysitter' ), $product->get_duration() );
						}
...
<?php else : ?>
	...
	<p><?php _e( 'No packages found', 'babysitter' ); ?></p>
