<?php
add_action('widgets_init', 'babysitter_tabs_load_widgets');

function babysitter_tabs_load_widgets()
{
	register_widget('Babysitter_Tabs_Widget');
}

class Babysitter_Tabs_Widget extends WP_Widget {
	
	function __construct() {
		$widget_ops = array(
			'classname' => 'tabbed-widget',
			'description' => __('Popular posts, Recent Post and Comments.', 'babysitter')
		);

		$control_ops = array('id_base' => 'babysitter_tabs-widget');

		parent::__construct('babysitter_tabs-widget', 'Babysitter - Tabs', $widget_ops, $control_ops);
	}
	...
	function widget($args, $instance)
	{
		...
		<div class="tabs">
			...
			<ul class="nav nav-tabs">
				<li class="active"><a href="#tab-popular" data-toggle="tab"><?php echo __('Popular', 'babysitter'); ?></a></li>
				<li><a href="#tab-recent" data-toggle="tab"><?php echo __('Recent', 'babysitter'); ?></a></li>
			...
			<div class="tab-content">
				...
				<div id="tab-popular" class="tab-pane fade in active">
					...
					if($orderby == 'Highest Comments') {
						...
						$order_string = '&meta_key=babysitter_post_views_count&orderby=meta_value_num';
					}
				...
				<div id="tab-comments" class="tab-pane fade">
					<ul class="latest-posts-list">
						...
						<li>
							...
							<div class="post-item-holder">
								<div class="meta"><?php echo strip_tags($comment->comment_author); ?> <?php _e('says', 'babysitter'); ?>:</div>
								<h5 class="title">
									<a class="comment-text-side" href="<?php echo get_permalink($comment->ID); ?>#comment-<?php echo $comment->comment_ID; ?>" title="<?php echo strip_tags($comment->comment_author); ?> on <?php echo $comment->post_title; ?>"><?php echo babysitter_string_limit_words(strip_tags($comment->com_excerpt), 12); ?></a>
								</h5>
							</div>
						</li>
						<?php } ?>
	...
	function form($instance)
	{
		...
		<p>
			<label for="<?php echo $this->get_field_id('posts'); ?>"><?php echo _e('Number of popular posts:', 'babysitter') ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id('recent_posts'); ?>"><?php echo _e('Number of recent posts:', 'babysitter') ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id('comments'); ?>"><?php echo _e('Number of comments:', 'babysitter') ?></label>
