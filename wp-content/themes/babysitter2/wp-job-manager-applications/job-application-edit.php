<form class="job-manager-application-edit-form job-manager-form" method="post">
	...
	<fieldset class="fieldset-status">
		<label for="application-status-<?php esc_attr_e( $application->ID ); ?>"><?php _e( 'Application status', 'babysitter' ); ?>:</label>
		<div class="field">
	...
	<p>
		<a class="btn btn-sm btn-danger pull-right" href="<?php echo wp_nonce_url( add_query_arg( 'delete_job_application', $application->ID ), 'delete_job_application' ); ?>"><?php _e( 'Delete', 'babysitter' ); ?></a>
		<input type="submit" class="btn btn-secondary btn-sm" name="wp_job_manager_edit_application" value="<?php esc_attr_e( 'Save changes', 'babysitter' ); ?>" />
