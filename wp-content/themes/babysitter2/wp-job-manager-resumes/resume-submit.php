			...
			<?php if ( resume_manager_user_can_post_resume() ) : ?>
				...
				<?php foreach ( $resume_fields as $key => $field ) : ?>
					<fieldset class="fieldset-<?php esc_attr_e( $key ); ?>">
						<label for="<?php esc_attr_e( $key ); ?>"><?php echo $field['label'] . apply_filters( 'submit_resume_form_required_label', $field['required'] ? '' : ' <small>' . __( '(optional)', 'babysitter' ) . '</small>', $field ); ?></label>
						<div class="field">
