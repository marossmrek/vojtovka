...
if (!function_exists('cta_shortcode')) {
	function cta_shortcode($atts, $content = null) {
		...
		$output = '<section class="call-to-action '.$centered.' '.$fullwidth.' '.$overlay.' cta__overlay-opacity-'.$overlay_opacity.' '.$custom_class.' cta-overlay-color__'.$overlay_color.'" '.$cta_bg.'>';
...
if (!function_exists('pricing_shortcode')) {
	function pricing_shortcode($atts, $content = null) {
		...
		$output = '<div class="pricing-table-plan '.$active.'">';
...
if (!function_exists('dropcap_shortcode')) {
	function dropcap_shortcode($atts, $content = null) {
		...
		$output = '<span class="dropcap dropcap__color-'.$type.'">';
		$output .= do_shortcode($content);
		$output .= '</span>';
...
if (!function_exists('section_shortcode')) {
	function section_shortcode($atts, $content = null) {
		...
		$output = '<div class="section '.$type.' '.$class.' section-overlay__'.$overlay.' section-overlay-color__'.$overlay_color.' section-overlay-opacity-'.$overlay_opacity.'" '.$section_bg.'>';
		...
		foreach($latest as $k => $post) {
			...
			if ( function_exists( 'wpml_get_language_information' ) ) {
				...
				$output .= '<div class="post-excerpt">';
					$output .= '<p>';
						$output .= babysitter_string_limit_words($excerpt,$words_num);
					$output .= '</p>';
				$output .= '</div>';
	...
	function shortcode_portfolio($atts, $content = null) {
			...
			foreach($latest as $k => $post) {
				...
				if ( function_exists( 'wpml_get_language_information' ) ) {
					...
					$thumb   = get_post_thumbnail_id();
					$img_url = wp_get_attachment_url( $thumb,'full'); //get img URL
					$short_info = get_post_meta(get_the_ID(), 'babysitter_short_info', true);
...
if (!function_exists('jobs_slider_shortcode')) {
	function jobs_slider_shortcode($atts, $content = null) {
		...
		<div class="alert alert-warning">
			<?php _e('<strong>Resume Manager</strong> add-on is missed. Please install and activate it.', 'babysitter'); ?>
		</div>
...
if (!function_exists('jobs_feed_shortcode')) {
	function jobs_feed_shortcode($atts, $content = null) {
		...
		<div class="alert alert-warning">
			<?php _e('<strong>WP Job Manager</strong> add-on is missed. Please install and activate it.', 'babysitter'); ?>
		</div>
